...
void 	ft_printtr_inord(t_bitree *node, void (*f)(const char *str))
{
	//create in order that takes pointer to function
	//apply said function to tree inorder, preorder, etc.
	if (node == NULL || node->item == NULL)
		return ;
	ft_printtr_inord(node->left, f);
	f(node->item); 
	ft_printtr_inord(node->right, f);
}
...
int		main(int argc, char **argv)
{
	...
	ft_printtr_inord(root, ft_putendl);
	ft_treedel_postord(root);
	arrelem_free(arr);
	printf("%d", printf("%d", printf("%d", 100011)));
	test_new(argc, argv);
	return (0);
}
